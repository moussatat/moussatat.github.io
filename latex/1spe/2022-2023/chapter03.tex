...
\begin{eBox}
$ax^2+bx+c=0$, avec $a\neq0$ est la forme standard d'une équation quadratique d'inconnue $x$.
\end{eBox}
...
\begin{enumerate}[label={\arabic*)\rule{0pt}{1.8em}},leftmargin=*]
	\item La forme standard de l'équation $5x^2=6x-8$ est $\ldots\ldots x^2+ \ldots\ldots x+\ldots\ldots =0$.
...
\begin{exercise}\emoji{cactus} Soit l'équation $kx^2-k(x+2)=x(2x+3)+1$ d'inconnue $x$.
	\begin{enumerate}[label=\alph*), leftmargin=*]
		\item Ecrire cette équation sous la forme standard $a x^2+bx + c=0$.
		\vfill 	
...
\begin{exercise}\label{chapter01:exercice:equation:factorise} Factoriser à l'aide de facteurs commun ou différence de carrés puis résoudre. 
	...
	\end{multicols} 
\end{exercise}  
\begin{proof}[solutions des exercices \ref{chapter01:exercice:equation:racines} et \ref{chapter01:exercice:equation:factorise}]\hfill  \vspace{-1em}
...
\begin{exercise} Sans résoudre, entourez l'équation quadratiques ayant deux solutions réelles distinces. 
	\vspace{-1em}
	\begin{multicols}{4}
		\begin{enumerate}[label={$(E_{\arabic*})$}, leftmargin=*]
			\item $x^2+1=0$ 
...
\begin{exercise} Complétez les espaces.
	...
	\begin{enumerate}[label={\arabic*)\rule{0pt}{1.8em}},leftmargin=*] 
		...
		Si $\Delta<$, l'équation \dotfill solutions réelles.\\\medskip
		Si $\Delta\geqslant 0$, les solutions de l'équation sont $r_1=$\dotfill et $r_2=$\dotfill.\medskip
	...
	\end{enumerate}
\end{exercise}
\begin{exercise} Sans cherchez à les résoudre donner le nombre de solutions dans $\mathbb{R}$ des équations suivantes d'inconnue $x$.	
	\vspace{-1em}
	\begin{multicols}{3}
		\begin{enumerate}[label={$(E_{\arabic*})$}, leftmargin=*]
			\item $2x^2+3x=4$ 
	...
	\end{multicols}   
\end{exercise}
\begin{exercise}  Sans chechez à la résoudre,  donner le nombre de solution de l'équation   $x^2-2mx+4(m-1)=0$ d'inconnue $x$.
\end{exercise} 
...
\begin{tikzpicture}
	%\tikzset{  double style/.style = {double,very thick,double distance=2pt}}
	\tikzset{  double style/.style = {double,thick,double distance=2pt}}
	\tkzTabInit[lgt = 2.0, espcl =1.4, color, colorC = blue!15, colorL = green!15]{$x$ / 1 ,   \scriptsize$3x^2-4x+7$/ 1.0 ,   \scriptsize$(2x+1)^2$/ 1.0 , \scriptsize$\frac{3x^2-4x+7}{2x+1}$/1.2 }{$-\infty$ ,$\quad$ ,$\quad$ ,$\quad$, $+\infty$} 
	\tkzTabLine{t, ,t, ,t, ,t, ,t}% 
	\tkzTabLine{t, ,t, ,t, ,t, ,t}% 
	\tkzTabLine{t, ,t, ,t, ,t, ,t}% 
\end{tikzpicture}  
...
\begin{spacing}{2.0}
La parabole  $\mathscr{P}\colon y=0.25x^2$ et la droite $d\colon y =0.5x+m$ ont un unique point commun $M(x;y)$. 
...
$\Delta=$\dotfill $m$ vérifie \dotfill 
...
\end{exercise}
\begin{exercise} \hfill \\
	 Chaque membre d'un group doit envoyer une photo à tous les autres membres. Un total de 90 photos ont été échangés.   
...
\begin{exercise} \hfill \\
	Un   rectangle a pour aire \SI{225}{\centi\meter^2}. Sa longueur est \SI{16}{\centi\meter} de plus que ca largeur. Trouvez la largeur.
\end{exercise} 
%9 et 25
...
%\begin{exercise}[un problème d'optimisation]\hfill \\Soit $ABCD$ un carré de coté $5$ cm. $E$, $F$, $G$ et $H$ sont des points appartenant aux cotés du carré tels que $AE$=$BF$=$CG$=$DH$=$x$. On admet que $EFGH$ est aussi un carré.
%	
%\noindent \parbox{0.75\linewidth}{ 
%	\begin{enumerate}[label=\alph*),leftmargin=*]
