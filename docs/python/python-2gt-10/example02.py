...
def isqrt(n) :
    i = 0
    while i**2 <= n:                # pensez aux carrés parfaits 
        print(f"{i}*{i}={i**2}<{n}")   
        i = i + 1
    print(f"{i}*{i}={i**2}>{n}")        
    return i-1                      # il faut renvoyer la valeur précédente de i 
...
def tests():  
    for i in range(100) :
        exp = floor(sqrt(i))     
        val = isqrt(i)    
        msg = f"l'instruction isqrt({i}) doit renvoyer {exp} et non {val}" # La totale 
        assert val == exp, msg
