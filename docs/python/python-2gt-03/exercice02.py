...
def tests():  
    ...
    val_b = b
    val_s = s 
    msg_a = f"a devrait être {exp_a} et non pas {val_a}"
    msg_b = f"b devrait être {exp_b} et non pas {val_b}"
    msg_a = f"s devrait être {exp_s} et non pas {val_s}" 
     
    assert val_a == exp_a, msg_a  
    assert val_b == exp_b, msg_b  
    assert val_s == exp_s, msg_s     
