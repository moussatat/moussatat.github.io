...
def s(n) :
	...
	for loop in range(1,n+1) :                 # i prend les valeurs 1 à 20 
		accumulateur = accumulateur + 1/2**loop    # multiplier l'accumulateur par i 
	return accumulateur  	
...
def tests():  
    for n in range(1,100) :
        ...
        for i in range(1,n+1) :                
            exp = exp + 1/2**i             
        val = s(n) 
        msg = f"l'instruction produit() doit renvoyer {exp} et non {val}" # La totale 

        assert abs(val - exp)<1e-5, msg
