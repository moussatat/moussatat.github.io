...
def somme() :
	accumulateur = 0                        # initialisation de l'accumulateur
	for i in range(100) :                   # i prend les valeurs 0 à 99 
		accumulateur = accumulateur + i     # augmenter accumulateur de i 
	return accumulateur 
...
def somme() :
	accumulateur = 0                        # initialisation de l'accumulateur
	for i in range(101) :                   # i prend les valeurs 0 à 99 
		accumulateur = accumulateur + i**2     # augmenter accumulateur de i 
	return accumulateur  	
...
def tests():  
    ...
    for i in range(101) :           
        exp = exp + i**2      
    val = somme() 
    msg = f"l'instruction somme() doit renvoyer {exp} et non {val}" # La totale 
