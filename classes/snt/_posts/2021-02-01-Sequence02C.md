...
en temps réel les modifications que vous allez apporter à votre texte, en intégrant des balises
 ```html``` et ```css```.
1. Ouvrir le dossier [nextcloud](https://cloud-grenoble.beta.education.fr/s/3L3zp62J8p69o36). Choisir un fichier quelconque et recopier le texte inclus (titre compris) entre les balises ```<body>``` et ```</body>```.
...
1. Les noms des deux fichiers indiqueront la **date** et noms des membres de votre groupe et sera téléversé dans le dossier réponses [nextcloud](https://cloud-grenoble.beta.education.fr/s/TH6YcZDEbJqmyxH)
